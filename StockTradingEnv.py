...
class StockTradingEnv(gym.Env):
    def __init__(self, data):
        ...
        self.trade_fee = 0.0  # 거래 수수료 
        self.current_shares = 0  # 현재 보유 주식 수
        self.min_order_size = 1  # 최소 주문 크기 (1주)
    ...
    def step(self, action):
        ...
        portfolio_value_before = self.current_cash + (self.current_shares * current_price)
        self.current_cash, self.current_shares, final_action = self.execute_trade(action)
        self.portfolio_value = self.current_cash + (self.current_shares * current_price)
        self.record_roe()
